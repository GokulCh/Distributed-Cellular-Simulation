...
def main():
    ...
    parser.add_argument(
        "--balance", action="store_true", help="Enable dynamic load balancing"
    )
    parser.add_argument(
        "--procs",
        type=int,
        default=1,
        help="Number of processes (ignored, set by mpiexec)",
    )
    parser.add_argument(
        "--save", action="store_true", help="Save grid snapshots for visualization"
    )
    parser.add_argument(
        "--fire-pos",
        choices=["center", "top"],
        default="center",
        help="Initial fire position",
    )
    ...
    # 1D Domain Decomposition: Split rows among ranks
    ...
    # Determine if fire starts in my local grid
    ...
    for step in range(args.steps):
        # 1. Exchange ghost cells (non-blocking)
        requests = comm_obj.start_ghost_exchange(grid)

        # 2. Wait for exchange to complete
        ...
        # 4. Check load imbalance every 5 steps
        if args.balance and step % 5 == 0:
            balancer.redistribute(grid)

        # 5. Log progress and save snapshots
        if step % 10 == 0:
            total_burning = comm_obj.comm.reduce(
                np.sum(grid.data == BURNING), op=MPI.SUM, root=0
            )
