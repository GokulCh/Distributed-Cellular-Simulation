...
class Communicator:
    ...
    def start_ghost_exchange(self, grid):
        """
        Initiate non-blocking ghost cell exchange.
        Returns a list of MPI Request objects to wait on.
        """
        ...
        # Prepare data for sending (top and bottom rows)
        ...
        if self.up != MPI.PROC_NULL:
            # Send my top row, receive into top ghost buffer
        ...
        if self.down != MPI.PROC_NULL:
            # Send my bottom row, receive into bottom ghost buffer
            req_s_down = self.comm.Isend(send_down, dest=self.down, tag=TAG_UP)
            requests.append(req_s_down)

            # Recv from DOWN
            req_r_down = self.comm.Irecv(
                self.recv_down_buf, source=self.down, tag=TAG_DOWN
            )
    ...
    def end_ghost_exchange(self, grid, requests):
        """
        Complete the ghost exchange and update the grid's ghost layers.
        """
    ...
    def gather_grid(self, grid):
        """
        Collect the full grid state on rank 0 for visualization/IO.
        """
        ...
        # Rank 0 reconstructs the global grid
        if self.rank == 0:
            # Total rows
            ...
            if self.size > 1:
                self.comm.Gatherv(
                    local_data,
                    [full_grid, counts, displacements_bytes, MPI.SIGNED_CHAR],
                    root=0,
                )
            else:
